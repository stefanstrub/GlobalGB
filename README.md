...
```
To run all batches in parallel using slurm. The highest batch index is determined by int(len(frequencies_even)/batch_size) + 2* batch_size. For the last two batches the batch size is reduced to 1.
```bash
#!/bin/bash
#SBATCH -n 1
#SBATCH --time=4:00:00
